...
import getRelatedStateOpps from '@salesforce/apex/CERelatedOpps.getRelatedStateOpps';
import getRelatedStateOppsCount from '@salesforce/apex/CERelatedOpps.getRelatedStateOppsCount';
...
export default class CeRelatedStateOpps extends LightningElement {    
    @api recordId;
    @api rowLimit = 25;
    columns = columnsDefs;
    ...
    getTotalNumberOfRows() {
        return getRelatedStateOppsCount({ expanCityRecId: this.recordId })
            .then(result => {
    ...
    async fetchData(){
        ...
        try {
            const result = await getRelatedStateOpps({ expanCityRecId: this.recordId, limitSize: this.rowLimit, offset: this.rowOffSet, sortField: this.apexSortBy, sortDir: this.apexSortDirection });
            let tempResult = [];
    ...
    showSuccessMessage(oppName) {
        const eventSuccess = new ShowToastEvent({
            ...
            message: 'Once this page is refreshed this Opportunity will be dropped from Local Opps Tab.',
            variant: 'success',
            mode: 'sticky'
        });
