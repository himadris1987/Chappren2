...
import getRelatedContacts from '@salesforce/apex/CERelatedOpps.getRelatedContacts';
import getRelatedContactsCount from '@salesforce/apex/CERelatedOpps.getRelatedContactsCount';
//import columns
...
export default class CeRelatedContacts extends LightningElement {
    @api recordId;
    @api rowLimit = 25;
    columns = columnsDefs;
    ...
    getTotalNumberOfRows() {
        return getRelatedContactsCount({ expanCityRecId: this.recordId })
            .then(result => {
    ...
    async fetchData() {
        ...
        try {
            const result = await getRelatedContacts({ expanCityRecId: this.recordId, limitSize: this.rowLimit, offset: this.rowOffSet, sortField: this.apexSortBy, sortDir: this.apexSortDirection });
            let tempResult = [];
...
// @wire(getRelatedCityContacts, { expanCityRecId: '$recordId' })
// getAccountList({ data, error }) {
//     this.showLoadingSpinner = true;
//     if (data) {
//         data = JSON.parse(JSON.stringify(data));
//         data.forEach(conRec => {
//             conRec.ContactNameUrl = '/lightning/r/Contact/' + conRec.Id + '/view';
//             conRec.AccountUrl = '/lightning/r/Account/' + conRec.AccountId + '/view';
//             if (conRec.AccountId) {
...
//         this.result = data;
//         this.error = undefined;
//         this.totalItems = (data.length == 1 ? '1 item.' : data.length + ' items.');
