...
import getLocalAccounts from '@salesforce/apex/CERelatedOpps.getLocalAccounts';
import getLocalAccountsCount from '@salesforce/apex/CERelatedOpps.getLocalAccountsCount';
//import columns
...
export default class CeLocalAccounts extends LightningElement {
    @api recordId;
    @api rowLimit = 25;
    columns = columnsDefs;
    ...
    getTotalNumberOfRows() {
        return getLocalAccountsCount({ expanCityRecId: this.recordId })
            .then(result => {
    ...
    async fetchData(){
        ...
        try {
            const result = await getLocalAccounts({ expanCityRecId: this.recordId, limitSize: this.rowLimit, offset: this.rowOffSet, sortField: this.apexSortBy, sortDir: this.apexSortDirection });
            let tempResult = [];
